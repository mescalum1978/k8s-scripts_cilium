...
# - /etc/network/interfaces configured before running this script including routing
# - /etc/resolv.conf is configured before running this script with a DNS domain and at lease one DNS server
# - A records created in an authoritive DNS Zone 
...
echo "deb [signed-by=/etc/apt/trusted.gpg.d/kubernetes.gpg] https://pkgs.k8s.io/core:/stable:/v1.35/deb/ /" | sudo tee /etc/apt/sources.list.d/kubernetes.list
sudo apt update
